...
std::vector<Item*> MonsterType::createLootItem(const LootBlock& lootBlock)
{
	...
	while (itemCount > 0) {
		...
		if (tmpItem->getWeaponType() != WEAPON_NONE && tmpItem->getWeaponType() != WEAPON_AMMO && tmpItem->getWeaponType() != WEAPON_SHIELD)
		{
			int mejora = 0;
			int chance = rand() % 100 + 1;
			if (chance > 50) {
				//50 % para que salga un arma mejorada
				chance = rand() % 100 + 1;
				if (chance < 50) {
					//50% para que salga con + 1
					mejora = 1;
				}
				else if (chance > 50 && chance < 75) {
					//25% para que salga con  + 2
					mejora = 2;
				}
				else{
					//25% para que salga con + 3
					mejora = 3;
				}
				tmpItem->setIntAttr(ITEM_ATTRIBUTE_ATTACK, tmpItem->getAttack() + mejora);
				if (tmpItem->hasAttribute(ITEM_ATTRIBUTE_DESCRIPTION)) {
					std::string description = tmpItem->getStrAttr(ITEM_ATTRIBUTE_DESCRIPTION);
					description.append("This weapon has +").append(std::to_string(mejora)).append(" of attack.");
					tmpItem->setStrAttr(ITEM_ATTRIBUTE_DESCRIPTION, description);
				}
				else {
					std::string description = "This weapon has +";
					description.append(std::to_string(mejora)).append(" of attack.");
					tmpItem->setStrAttr(ITEM_ATTRIBUTE_DESCRIPTION, description);
				}
			}
		}
