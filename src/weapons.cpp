...
{
	//std::cout << "Use Weapon: " << item->getID() << std::endl;
...
{
	//std::cout << "Weapons -- doCombat:: line 411: " << std::endl;
	if (scripted) {
		LuaVariant var;
...
bool WeaponWand::configureEvent(const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("ignoreDefense"))) {
		std::cout << "ignoreDefense" <<pugi::cast<int32_t>(attr.value()) << std::endl;
	}
	return true;
}
...
{
	//std::cout << "Min Change: " << minChange << std::endl;
