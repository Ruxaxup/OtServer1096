...
void LuaScriptInterface::registerFunctions()
{
	...
	registerGlobalMethod("getMonsterInfo", LuaScriptInterface::luaGetMonsterInfo);
...
int LuaScriptInterface::luaGetMonsterInfo(lua_State* L)
{
	//getMonsterInfo(name)
	const MonsterType* mType = g_monsters.getMonsterType(getString(L, 1).c_str());
	if (!mType)
	{
		reportErrorFunc(getErrorDesc(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushnil(L);
	...
	int lootSize = mType->lootItems.size();
	std::vector<LootBlock> lootItems = mType->lootItems;
	lua_createtable(L, lootSize, 0);
	int index = 0;
	for (auto it = lootItems.rbegin(), end = lootItems.rend(); it != end; ++it) 
	{
		lua_createtable(L, 0, 2);
		setField(L, "itemid", it->id);
...
{
	// player:setFightMode(accountType)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
...
int LuaScriptInterface::luaCombatExecute(lua_State* L)
{
	...
	//std::cout << "luaCombatExecute" << std::endl;
	Combat* combat = getUserdata<Combat>(L, 1);
	if (!combat) {
		pushBoolean(L, false);
		return 1;
